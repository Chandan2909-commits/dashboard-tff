...
const CLOSED_QUERIES_API_URL = 'https://script.google.com/macros/s/AKfycbyJPEdD-3RiP1pzKPCWeJ76hkFOSIZ8nzzP4prihhgE88f94EXtrkoKqpQTii-ybVOq/exec';
...
let closedEmailSet = new Set(); // "email"       — last-resort (#ERROR! phone)
...
function isEntryClosed(email, phone, date) {
    ...
    return (
        ...
        closedEmailSet.has(email)             // L3: email only  (#ERROR! phone)
    );
}
...
function loadClosedQueriesFromStorage() {
    ...
    if (savedKeys) {
        ...
        closedQueries.forEach(key => {
            ...
            if (parts.length >= 1) closedEmailSet.add(parts[0]);
...
async function postClosedQueryToSheet(detail) {
    ...
    try {
        await fetch(CLOSED_QUERIES_API_URL, {
            method: 'POST',
...
async function syncClosedQueriesFromSheet() {
    ...
    if (sheetRows && sheetRows.length > 0) {
        ...
        sheetRows.forEach(row => {
            ...
            closedEmailSet.add(email);                               // Level 3: email only
        ...
        queriesResolved = closedEmailSet.size; // unique emails = most accurate count
        saveClosedQueriesToStorage();
        console.log(`[sync] ✔ ${closedEmailSet.size} closed email(s) loaded from Google Sheet.`);
    } else {
